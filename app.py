...
@app.route('/capture_mobile', methods=['POST'])
def capture_mobile():
    state = request.form.get('state', 'no_hecha')
    file = request.files.get('frame')

    if file is None:
        return jsonify({"status": "error", "msg": "No se recibió imagen"})

    os.makedirs(f"dataset/{state}", exist_ok=True)
    filename = f"{state}_{len(os.listdir(f'dataset/{state}'))}.jpg"
    filepath = f"dataset/{state}/{filename}"
    file.save(filepath)
